...
static void Read_NODE_COORD_SECTION(float **coordenadas);
...
void ReadProblem(float **coordenadas, int n)
{
    int i, K;
    char *Line, *Keyword;

    if (!(ProblemFile = fopen("lala.tsp", "r")))
    ...
    Read_NODE_COORD_SECTION(coordenadas);
...
static void Read_NODE_COORD_SECTION(float **coordenadas)
{
	...
	N->X = coordenadas[i-1][0];
	N->Y = coordenadas[i-1][1];
